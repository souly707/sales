    ...
    <div class="col-md-12">
        ...
        <div class="card mb-4">
            <div class="card-header py-3 d-flex justify-content-between">
                <h5 class="m-0 font-weight-bold">تعديل وحدة قياس [{{ $inv_uom->name }}]</h5>
            ...
            <div class="card-body">
                ...
                <form id="formAccountSettings" action="{{ route('backend.inv_uoms.update', $inv_uom->id) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="mb-3 col-md-4">
                            <label for="name" class="form-label font-weight-bold">اسم وحدة القياس</label>
                            <input class="form-control @error('name') is-invalid @enderror" type="text" id="name"
                                name="name" value="{{ old('name', $inv_uom->name) }}" autofocus />
                        ...
                        <div class="mb-3 col-md-4">
                            <label class="form-label" for="is_master">نوع وحدة القياس</label>
                            <select name="is_master" id="is_master"
                                class="form-select @error('is_master') is-invalid @enderror">
                                <option value="1" {{ old('is_master', $inv_uom->is_master)=='1' ? 'selected' : null }}>
                                    رئيسية
                                </option>

                                <option value="0" {{ old('is_master', $inv_uom->is_master)=='0' ? 'selected' : null }}>
                                    تجزئة
                                </option>
                            </select>
                            @error('is_master')
                        ...
                        <div class="mb-3 col-md-4">
                            ...
                            <select name="active" id="active" class="form-select @error('active') is-invalid @enderror">
                                <option value="1" {{ old('active', $inv_uom->active)=='1' ? 'selected' : null }}>
                                    مفعل
                                </option>

                                <option value="0" {{ old('active', $inv_uom->active)=='0' ? 'selected' : null }}>
                                    غير مفعل
                                </option>
                            </select>
                            @error('inv_uoms')
